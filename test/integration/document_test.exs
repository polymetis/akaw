defmodule Integration.DocumentTest do
  ...
  setup_all do
    ...
    Akaw.delete_db(TestHelper.server, @all_docs_db)
    ...
    Akaw.create_db(TestHelper.server, @all_docs_db)
  ...
  test "creating doc with existing id should fail", %{db: db} do
    doc_id = "EXISTING_ID"
    doc = %{"key" => "value", "_id" => doc_id}
    {:ok, _} = Akaw.save_doc(db, doc)
    assert {:error, :conflict} == Akaw.save_doc(db, doc)
  end
  ...
  test "update doc", %{db: db} do
    ...
    {:ok, update} = Akaw.save_doc(db, doc)
    assert update["_id"] == doc_id
  end

  test "open doc", %{db: db} do
    doc = Akaw.open_doc(db, %{id: @existing_doc_id})
  ...
  test "open not existing doc", %{db: db} do
    assert {:error, :not_found} == Akaw.open_doc(db, %{id: "not_existing_doc_id"})
  end

  test "doc exists", %{db: db} do
    assert Akaw.doc_exists?(db, @existing_doc_id)
  end

  test "doc does not exist", %{db: db} do
    refute Akaw.doc_exists?(db, "not_existing_doc_id")
  end

  test "all docs" do
    {:ok, db} = Akaw.open_db(TestHelper.server, @all_docs_db)
  ...
  test "delete doc", %{db: db} do
    ...
    {:ok, resp_doc} = Akaw.save_doc(db, doc)
    {:ok, resp} = Akaw.delete_doc(db, %{id: resp_doc["_id"], rev: resp_doc["_rev"]})
    assert resp["ok"]
    refute Akaw.doc_exists?(db, resp_doc["_id"])
  end
  ...
  test "lookup_doc_rev", %{db: db} do
    ...
    doc = %{"key" => "value", "_id" => doc_id}
    {:ok, resp_doc} = Akaw.save_doc(db, doc)
    {:ok, rev} = Akaw.lookup_doc_rev(db, doc_id)
  ...
  test "put attachment", %{db: db} do
    ...
    {:ok, response} = Akaw.put_attachment(db, %{id: @existing_doc_id}, attachment)
  ...
  test "delete attachment without doc revision", %{db: db} do
    ...
    {:ok, response} = Akaw.delete_attachment(db, %{id: doc_id}, "file.txt")
  ...
  test "delete attachment with doc revision", %{db: db} do
    ...
    {:ok, response} = Akaw.delete_attachment(db, %{id: doc_info["id"], rev: doc_info["rev"]}, "file2.txt")
    assert response["id"] == doc_id
    assert String.contains? response["rev"], "3-"
  end
