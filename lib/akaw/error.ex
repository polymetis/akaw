defmodule Akaw.Error do
  ...
  The failure shapes:
    ...
    * **Transport** (DNS, connection refused, timeout, …) — `status` is
      ...
      so you can re-examine the source error if needed — a
      `%Req.TransportError{}` on plain requests, the raw Finch/Mint
      exception on streaming paths (which bypass Req's error step).
    ...
    * **Broken auth contracts** — `status` is `nil`, `error` is
      ...
      (`Akaw.SessionServer` absorbed a raise or exit from a refresh
      attempt). `"refresh_exception"` fills `body` with
      `%{exception: e, stacktrace: trace}`, where `e` is the exception
      struct — or a `{kind, value}` tuple for a caught exit or throw.

  For transport failures — streaming or not — treat `reason` as
  display-only and branch on `body.exception`. `reason` is generated by
  the transport, and its text can change between Erlang/OTP or Finch
  releases without the underlying condition changing at all.
  ...
  ## Fields
    ...
    * `:body`   — raw decoded response body (HTTP errors) or
      `%{exception: e}` (transport errors; `"refresh_exception"` adds
      `:stacktrace`, and its `:exception` may be a `{kind, value}`
      tuple for a caught exit or throw)
